...
if($this->options['qsn_options']['is_show_marking']){//允许评分，加载评分js
$print_head_param['js'][] = '/admin/paper/manual_marking.js';
}
$this->printHead($print_head_param);
?>
<style>
...
$(document).ready(function(){
	...
	<?php //还原用户答案
		...
		examDisplayUserAnswer(<?php echo json_encode($this->papr_info['user_exam_answer']);?>,<?php echo $this->options['revert_user_exam_answer']?'true':'false';?>);
	<?php }?>
	<?php //还原答案状态
	...
	<?php }?>
	<?php if($this->options['qsn_options']['is_show_remark']){//还原评语
	?>
		examDisplayRemark(<?php echo json_encode($this->papr_info['user_exam_info']);?>);
	<?php
	}
	?>
	...
	$('#footer a').attr('href','#');
	<?php }?>
	<?php //启用考试监控exam_disable_monitor_flg
...
<input type="hidden" id="exam_times" value="<?php echo isset($this->papr_info['user_exam_times'])?$this->papr_info['user_exam_times']:'1'?>" />
<input type="hidden" id="user_id" value="<?php echo isset($this->papr_info['user_id'])?$this->papr_info['user_id']:''?>" />
<input type="hidden" id="papr_id" value="<?php echo $this->papr_info['papr_id']?>" />
<?php if(isset($this->papr_info['exam_id'])){?><input type="hidden" id="exam_id" value="<?php echo $this->papr_info['exam_id']?>" /><?php }?>
<input type="hidden" id="exam_type" value="<?php echo isset($this->papr_info['exam_type'])?$this->papr_info['exam_type']:''?>" />
<input type="hidden" id="exam_blank_flg" value="<?php echo isset($this->papr_info['exam_blank_flg'])?$this->papr_info['exam_blank_flg']:''?>" />
<input type="hidden" id="exam_cloze_flg" value="<?php echo isset($this->papr_info['exam_cloze_flg'])?$this->papr_info['exam_cloze_flg']:''?>" />
<input type="hidden" id="exam_submit_display_result" value="<?php echo isset($this->papr_info['exam_submit_display_result'])?$this->papr_info['exam_submit_display_result']:''?>" />
<div id="container" class="box block_12">
	<div class="box_inner">
	<?php if($this->options['is_display_exam_info']){?>
		<div class="paper_header">
			...
			<div class="col_right">
				...
				<div class="left">
					<ul>
						<li><?php echo L::getText('考试时间', array('file'=>__FILE__, 'line'=>__LINE__))?>：<span class="col_9"><?php echo isset($this->papr_info['exam_begin_tm'])?$this->papr_info['exam_begin_tm']:''?> ~ <?php echo isset($this->papr_info['exam_end_tm'])?$this->papr_info['exam_end_tm']:''?></span></li>
						<li><?php echo L::getText('当前用户', array('file'=>__FILE__, 'line'=>__LINE__))?>：<span class="col_9"><?php echo isset($this->papr_info['user_name'])?$this->papr_info['user_name']:''?></span></li>
						<li><?php echo L::getText('所属组', array('file'=>__FILE__, 'line'=>__LINE__))?>：<span class="col_9"><?php echo isset($this->papr_info['user_group_name'])?$this->papr_info['user_group_name']:''?></span></li>
					</ul>
				</div>
				<?php if($this->options['is_show_user_score']){?>
		  <div class="lsat_score_text_box"><div style="float:left ;"><?php echo L::getText('您的成绩为', array('file'=>__FILE__, 'line'=>__LINE__))?></div>

          <div class="last_score" id="user_scoring_div"><?php echo $this->papr_info['user_exam_scoring']?></div><div class="lsat_score_text"><?php echo L::getText('分', array('file'=>__FILE__, 'line'=>__LINE__))?></div></div>
		<?php }?>
				<div class="right">
					<ul>
						<li><?php echo L::getText('答卷时间', array('file'=>__FILE__, 'line'=>__LINE__))?>：<?php echo isset($this->papr_info['user_group_name'])?$this->papr_info['exam_total_tm'] == 0?L::getText('不限', array('file'=>__FILE__, 'line'=>__LINE__)):$this->papr_info['exam_total_tm'].L::getText('分', array('file'=>__FILE__, 'line'=>__LINE__)):'' ;?></li>
			...
			<div class="contnet" id="exam_content_notice_div">
				<div class="inner">
					...
					<div class="button_area button_center ~button_left ~button_right">
						<div class="inner_box">
							<a href="javascript:void(0)" class="btn btn_disable" onclick="examToggleExamNotice();return false;" ><span class="timeout_txt" id="exam_notice_cd_span">(60)</span><?php echo L::getText('确认已读', array('file'=>__FILE__, 'line'=>__LINE__))?></a>
						</div>
					</div>
				</div>
			</div>
		...
		<div class="paper_filter" style="height:40px;width:1000px; z-index: 10;" id="exam_toolbar">
			<div class="inner">
			<?php //以下功能管理员不启用
		if(!$this->options['is_admin']){?><div class="paper_filter_timeout timeout_01 ~timeout_02 ~timeout_03"><?php echo L::getText('剩余时间', array('file'=>__FILE__, 'line'=>__LINE__))?>：<label id="exam_last_time_label">00:00:00</label></div>
		<?php }?>
		...
		$papr_qsn_type_count = count($this->papr_info['papr_qsn_type']);
		...
		foreach($this->papr_info['papr_qsn_type'] as $papr_qsn_type){
			...
			?>
			<div class="paper_list" name="papr_qsn_type_list_div" id="papr_qsn_type_list_div_<?php echo $papr_qsn_type['qsn_type_position']?>">
				<h1 class="title_level_1"><b><?php echo $papr_qsn_type['qsn_type_position']?>、<?php echo $papr_qsn_type['qsn_type_title']?>(<?php echo L::getText('每题', array('file'=>__FILE__, 'line'=>__LINE__))?><?php echo $papr_qsn_type['qsn_point'] ?><?php echo L::getText('分', array('file'=>__FILE__, 'line'=>__LINE__))?>，<?php echo L::getText('共', array('file'=>__FILE__, 'line'=>__LINE__))?><?php echo $papr_qsn_type['qsn_point']*$papr_qsn_type['qsn_count']?><?php echo L::getText('分', array('file'=>__FILE__, 'line'=>__LINE__))?>)</b></h1>
				<div class="paper_info">
				</div>
				<div id="paper_content_div" name="paper_content_div" style="width: 1001px">
				<?php 
				if(isset($this->papr_info['exam_qsn_random_flg'])&&$this->papr_info['exam_qsn_random_flg'] == '1'){
